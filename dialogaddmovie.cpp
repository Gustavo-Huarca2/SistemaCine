...
void DialogAddMovie::on_aceptar_clicked()
{
    ...
    try {
        if(title.isEmpty()){
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Titulo Vacio\t\t"),QMessageBox::Ok);
            return;
        }
        else if(duration.isEmpty()){
            //qDebug()<<"Duracion vacia";
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Duracion vacia\t\t"),QMessageBox::Ok);
            return;
        }
        else if(director.isEmpty()){
            //qDebug()<<"Director vacio";
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Director vacio\t\t"),QMessageBox::Ok);
            return;
        }
        else if(sinopsis.isEmpty()){
            //qDebug()<<"Sinopsis vacio";
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Sinopsis vacio\t\t"),QMessageBox::Ok);
            return;
        }
        else if(precio.isEmpty()){
            //qDebug()<<"Precio vacio";
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Precio vacio\t\t"),QMessageBox::Ok);
            return;
        }
        bool gencheck=false;
        for(int i=0;i<cg;i++){
        ...
        if(!gencheck){
            ...
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Seleccione al menos un genero\t\t"),QMessageBox::Ok);
            return;
        }else if(!idicheck){
            //qDebug()<<"Seleccione al menos un idioma";
            QMessageBox::warning(nullptr, tr("Alerta"), tr("Seleccione al menos un idioma\t\t"),QMessageBox::Ok);
            return;
        }
